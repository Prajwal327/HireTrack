...
export default function Dashboard() {
    ...
    return (
        <div className={`min-h-screen transition-all duration-700 font-sans ${isDarkMode ? 'bg-[#0f172a]' : 'bg-[#F8FAFC]'}`}>
            ...
            <div className="max-w-7xl mx-auto py-12 px-6 lg:px-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    {/* ASIDE: THE NEW ENTRY CARD (STYLIZED LIKE IMAGE) */}
                    <aside className="lg:col-span-4">
                        <div className={`p-10 rounded-[3.5rem] border transition-all duration-500 shadow-2xl bg-gradient-to-br from-slate-800 to-[#5459AC] text-white ${isDarkMode ? 'border-slate-700' : 'border-transparent'}   `}>
                            <div className="flex items-center justify-between mb-10">
                                <h2 className="text-4xl font-black tracking-tighter text-slate-800 dark:text-white">New Entry</h2>
                            ...
                            <form onSubmit={handleAdd} className="space-y-8">
                                <div className="space-y-2">
                                    ...
                                    <input type="text" value={form.company} required 
                                        className={`w-full p-5 rounded-[1.8rem] outline-none transition-all font-medium placeholder:text-slate-300 ${
                                            isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50/50 border-transparent'
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea value={form.notes} 
                                        className={`w-full p-5 rounded-[1.8rem] outline-none transition-all font-medium h-32 resize-none placeholder:text-slate-300 ${
                                            isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50/50 border-transparent'
                                        } focus:ring-2 focus:ring-emerald-500`}
                                        onChange={e => setForm({ ...form, notes: e.target.value })} placeholder="Key details..." />
                                </div>

                                <button className="w-full py-5 bg-gradient-to-r from-[#5459AC] to-slate-800 text-white rounded-[1.8rem] font-black tracking-widest text-xs hover:shadow-emerald-500/50 active:scale-95 transition-all flex items-center justify-center gap-2">
                    ...
                    {/* MAIN CONTENT: JOB LIST (STYLIZED LIKE IMAGE) */}
                                ...
                                <div key={job._id} className={`group p-8 rounded-[3rem] transition-all duration-500 backdrop-blur-md border transform hover:-translate-y-2 ${
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-6">
                                            <div className="w-16 h-16 rounded-[1.5rem] bg-gradient-to-br from-slate-800 to-[#5459AC] flex items-center justify-center text-slate-800 font-black text-3xl shadow-xl">
                                                {job.company[0]}
                                            </div>
                                            <div className="space-y-2">
                                                <div className="flex items-center gap-3">
                                                    <h3 className="font-black text-2xl tracking-tighter text-slate-800 dark:text-slate-800 leading-none">{job.company}</h3>
                                        ...
                                        <div className="flex items-center gap-3">
                                            <select 
                                                ...
                                                className={`appearance-none text-[10px] font-black uppercase tracking-widest px-6 py-3 rounded-full outline-none cursor-pointer border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}
                                            >
                                                {['Applied', 'Interview', 'Offer', 'Rejected'].map(s => <option key={s} value={s} className="bg-white text-slate-900  rounded-full">{s}</option>)}
                                            </select>
